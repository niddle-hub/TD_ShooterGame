...
UCLASS()
class TD_SHOOTERGAME_API USaveSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
public:
...
private:
	void CreateStreamingLevelObservers(const UWorld* World);
	...
	void DeserializeActor(AActor* Actor, const FActorSaveData* ActorSaveData) const;
	FString GetSaveFilePath(int32 SaveId) const;
	int32 GetNextSaveId() const;
	void OnActorSpawned(AActor* SpawnedActor) const;
	static void NotifyActorsAndComponents(AActor* Actor);
