...
void USaveSubsystem::LoadGame(const int32 SaveId)
{
...
UWorld* USaveSubsystem::GetWorld() const
{
	...
	if (const UObject* Outer = GetOuter(); IsValid(Outer))
	{
...
void USaveSubsystem::DeserializeLevel(ULevel* Level, const ULevelStreaming* StreamingLevel /*= nullptr*/)
{
	...
	for (TArray<AActor*>::TIterator ActorIterator = Level->Actors.CreateIterator(); ActorIterator; ++ActorIterator)
	{
		...
		const FActorSaveData* ActorSaveData = nullptr;
...
void USaveSubsystem::CreateStreamingLevelObservers(const UWorld* World)
{
	...
	const FOnActorSpawned::FDelegate OnActorSpawnedDelegate = FOnActorSpawned::FDelegate::CreateUObject(this, &USaveSubsystem::OnActorSpawned);
...
void USaveSubsystem::DeserializeActor(AActor* Actor, const FActorSaveData* ActorSaveData) const
{
...
void USaveSubsystem::OnActorSpawned(AActor* SpawnedActor) const
{
